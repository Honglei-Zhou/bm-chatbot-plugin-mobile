<template>
  <div ref="scrollList" :style="{backgroundColor: colors.messageList.bg}" class="sc-message-list" @scroll="handleScroll">
    <Message v-for="(message, idx) in messages" :message="message" :chat-image-url="chatImageUrl(message.author)" :author-name="authorName(message.author)" :key="idx" :colors="colors" :message-styling="messageStyling" @sendItem="sendItem"/>
    <Message v-show="showTypingIndicator !== ''" :message="{author: showTypingIndicator, type: 'typing'}" :chat-image-url="chatImageUrl(showTypingIndicator)" :colors="colors" :message-styling="messageStyling"/>
  </div>
</template>
<script>
