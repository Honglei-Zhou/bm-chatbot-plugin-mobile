<template>
  <div :style="messageColors" class="sc-message--file">
    <div class="sc-message--file-icon">
      <img :src="data.file.url" class="sc-image">
    </div>
    <div :style="messageColors" class="sc-message--file-name">
      <a :href="data.file.url ? data.file.url : '#'" target="_blank">{{ data.file.name || '' }}</a>
    </div>
    <div :style="messageColors" class="sc-message--file-text">{{ data.text }}<p v-if="data.meta" :style="messageColors" class="sc-message--meta">{{ data.meta }}</p></div>
  </div>
</template>
